...
const padding = (M) => {
    ...
    if (isOverflow)
        sizeLastBlock = 128; // Mの最後のブロックのバイトサイズが55bytesを超えていると1ブロックに格納できないため１ブロック追加する
    // Mを16進数文字列に変換
...
export const computeHash = (M) => {
    ...
    for (let i = 0; i < dividedM.length; i++) {
        const Mi = dividedM[i].match(/.{8}/g); // 64bytesのブロックをさらに4byteずつ刻んでいく
        if (!Mi)
